console.log('Script.js begins execution');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        let pendingLog = null; // Store log data while awaiting reflection
        ...
        const refGrowthScore = document.getElementById('ref-growth-score');
        const refGrowthVal = document.getElementById('ref-growth-val');
        const refGrowthText = document.getElementById('ref-growth-text');
        const refAutonomyScore = document.getElementById('ref-autonomy-score');
        const refAutonomyVal = document.getElementById('ref-autonomy-val');
        const refAutonomyText = document.getElementById('ref-autonomy-text');
        const refConnectionScore = document.getElementById('ref-connection-score');
        const refConnectionVal = document.getElementById('ref-connection-val');
        const refConnectionText = document.getElementById('ref-connection-text');
        ...
        // Global to track which log is being reflected upon
        let currentReflectionLogId = null;
        ...
        if (addLogBtn) {
            addLogBtn.addEventListener('click', () => {
                ...
                // Store in pendingLog instead of saving immediately
                pendingLog = {
                    id: Date.now().toString(),
                    idea,
                    test,
                ...
                if (logValuesDiscovery) {
                    ...
                    if (newValues.length > 0) {
                        pendingLog.valuesDiscovery = newValues;
                ...
                if (reflectionModal) {
                    ...
                    if (refGrowthVal) refGrowthVal.textContent = '5';
                    if (refGrowthText) refGrowthText.value = '';
                    if (refAutonomyScore) refAutonomyScore.value = 5;
                    if (refAutonomyVal) refAutonomyVal.textContent = '5';
                    if (refAutonomyText) refAutonomyText.value = '';
                    if (refConnectionScore) refConnectionScore.value = 5;
                    if (refConnectionVal) refConnectionVal.textContent = '5';
                    if (refConnectionText) refConnectionText.value = '';

                    reflectionModal.classList.remove('hidden');
                } else {
                    alert("Error: Reflection Modal element not found in DOM.");
                    // Fallback if modal missing
                    experimentLogs.unshift(pendingLog);
                    saveLogs();
                    renderLogs();
                    resetLogForm();
                }
        ...
        if (saveReflectionBtn) {
            saveReflectionBtn.addEventListener('click', () => {
                const reflection = {
                    growth: {
                        score: parseInt(refGrowthScore.value),
                        text: refGrowthText.value.trim()
                    },
                    autonomy: {
                        score: parseInt(refAutonomyScore.value),
                        text: refAutonomyText.value.trim()
                    },
                    connection: {
                        score: parseInt(refConnectionScore.value),
                        text: refConnectionText.value.trim()
                    }
                };

                if (currentReflectionLogId) {
                    // Update Daily Log (Chat Flow)
                    const logIndex = dailyLogs.findIndex(l => l.id === currentReflectionLogId);
                    if (logIndex !== -1) {
                        dailyLogs[logIndex].reflection = reflection;
                        saveDailyLogs();
                        // Optional: Re-render chat if reflection is shown there
                        if (currentHypoForChat) renderChatTimeline();
                    }
                    currentReflectionLogId = null;
                } else if (pendingLog) {
                    // Update Legacy Log (Direct Flow)
                    pendingLog.reflection = reflection;
                    experimentLogs.unshift(pendingLog);
                    saveLogs();
                    renderLogs();
                    resetLogForm();
        ...
        const linkSliderToVal = (slider, display) => {
            if (slider && display) {
        ...
        linkSliderToVal(refGrowthScore, refGrowthVal);
        linkSliderToVal(refAutonomyScore, refAutonomyVal);
        linkSliderToVal(refConnectionScore, refConnectionVal);
        ...
        if (chatSendBtn) {
            chatSendBtn.addEventListener('click', () => {
                ...
                if (currentEditingLogId) {
                    ...
                    // Trigger Reflection Modal
                    if (reflectionModal) {
                        currentReflectionLogId = newDailyLog.id;
        ...
        let activeChartType = 'needs'; // 'needs' | 'reflection'
        ...
        function renderAnalysisView() {
            ...
            updateRadarChart();
            if (activeChartType === 'reflection') {
                updateReflectionChart();
            }
        }

        function updateReflectionChart() {
            const ctx = document.getElementById('reflectionRadarChart');
            if (!ctx) return;
            ...
            // Helper to sum reflection
            const sumReflection = (log) => {
                if (log.reflection) {
                    scores.growth += (log.reflection.growth?.score || 0);
                    scores.autonomy += (log.reflection.autonomy?.score || 0);
                    scores.connection += (log.reflection.connection?.score || 0);
                }
            };

            dailyLogs.forEach(sumReflection);
            experimentLogs.forEach(sumReflection);

            const labels = ['成長 (Growth)', '自律 (Autonomy)', '貢献 (Connection)'];
            const dataValues = [scores.growth, scores.autonomy, scores.connection];

            if (reflectionChart) {
                reflectionChart.data.datasets[0].data = dataValues;
                ...
                reflectionChart = new Chart(ctx, {
                    ...
                    options: {
                        scales: {
                            r: {
                                beginAtZero: true,
                                // Dynamic scale based on totals, but at least 10
                                suggestedMax: Math.max(20, Math.max(...dataValues) + 5),
                                ticks: {
                                    stepSize: 5
                                }
                            }
        ...
        const toggleChartNeeds = document.getElementById('toggle-chart-needs');
        const toggleChartReflection = document.getElementById('toggle-chart-reflection');
        const needsCanvas = document.getElementById('needsRadarChart');
        const reflectionCanvas = document.getElementById('reflectionRadarChart');

        if (toggleChartNeeds && toggleChartReflection) {
            toggleChartNeeds.addEventListener('click', () => {
                activeChartType = 'needs';
                toggleChartNeeds.classList.add('active');
                toggleChartReflection.classList.remove('active');
                if (needsCanvas) needsCanvas.classList.remove('hidden');
                if (reflectionCanvas) reflectionCanvas.classList.add('hidden');
                renderAnalysisView();
            });

            toggleChartReflection.addEventListener('click', () => {
                activeChartType = 'reflection';
                toggleChartReflection.classList.add('active');
                toggleChartNeeds.classList.remove('active');
                if (reflectionCanvas) reflectionCanvas.classList.remove('hidden');
                if (needsCanvas) needsCanvas.classList.add('hidden');
                renderAnalysisView();
            });
        }
